 ...
 * @returns 
 */
export const useObserveStationContinuous = (ms: number, stationId: string, routeId: string) => {
  ...
  const getStationInfo = useCallback(async () => {
    try {
      ...
      const targetBusInfos = busInfos.filter((busInfo: BusInfoType) => busInfo.routeNumber === routeId)
